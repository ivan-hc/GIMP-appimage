...
mkdir -p $HOME/.cache
case "$1" in
	gimptool) $HERE/.local/share/junest/bin/junest -n 2> /dev/null -- gimptool "$@";;
	*) $HERE/.local/share/junest/bin/junest -n 2> /dev/null -- gimp "$@";;
esac
EOF
chmod a+x ./AppRun
...
mv ./*AppImage ./"$(cat ./$APP.AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g')"_"$VERSION""$VERSIONAUR"-archimage3-1-x86_64.AppImage
