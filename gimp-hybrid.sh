...
case "$1" in
	gimptool) $HERE/.local/share/junest/bin/junest -n -b "$BINDS" -- gimptool "$@";;
	*) $HERE/.local/share/junest/bin/junest -n -b "$BINDS" -- gimp "$@";;
esac
EOF
chmod a+x ./AppRun
...
LIBSAVED="babl dbus gdk gegl gir gjs gtk icons libalpm libasound libavcodec libdav libdl libgs libheif libjxl libmng libpthread librav libSvtAv libutil libwebp libwmf libXmu libXpm luajit paint poppler readline xml gio-launch-desktop librt.so libgtk-" # Enter here keywords or file/folder names to save in /usr/lib
...
mv ./*AppImage ./"$(cat ./$APP.AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g')"_"$VERSION"-Hybrid-with-python2-from-Debian-Buster-archimage3.4.1-x86_64.AppImage
