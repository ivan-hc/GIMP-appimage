...
APP=gimp-git
BIN="gimp"
DEPENDENCES="ca-certificates alsa-lib dav1d ffmpeg ghostscript gjs libunwind luajit nspr python qoi-headers-git rav1e sdl2 svt-av1 tar"
BASICSTUFF="binutils debugedit gzip gtk3 couchdb libjxl libheif aalib libmng ijs libxpm libgudev"
...
function _create_AppRun() {
	...
	case "$1" in
		gimptool) $HERE/.local/share/junest/bin/junest -n -b "$BINDS" -- $(echo $EXEC | sed s/-/tool-/g) "$@";;
...
function _extract_main_package() {
	...
	EXEC=$(grep -e '^Exec=.*' ./"$APP".AppDir/*.desktop | head -n 1 | cut -d "=" -f 2- | sed -e 's|%.||g')
	VERSION=$(./"$APP".AppDir/.local/share/junest/bin/junest -- $EXEC -v | tr ' ' '\n' | grep "GIMP" | sed 's/_/./g; s/GIMP.//g' | head -1)
	[ -z "$VERSION" ] && VERSION=$(cat ./base/.PKGINFO | grep pkgver | cut -c 10- | sed 's@.*:@@')
	mkdir -p deps
	rm -R -f ./deps/*
}
...
SHARESAVED="babl dbus gegl gir gjs gtk icons libalpm lua paint poppler readline thumbnailers xml" # Enter here keywords or file/directory names to save in both /usr/share and /usr/lib
...
LIBSAVED="babl dbus gdk gegl gir gjs gtk icons libalpm libasound libgudev libXpm libijs libmng libavcodec libmozjs libaa libheif libjxl libdav libdl libgs libheif libjxl libmng libpthread librav libSvtAv libunwind libutil libwebp libwmf libXmu libXpm lua paint poppler python readline xml gio-launch-desktop librt.so libgtk- $lib_browser_launcher" # Enter here keywords or file/directory names to save in /usr/lib
