...
DEPENDENCES="dbus gdb ghostscript gjs highway libgudev libunwind luajit nspr python python-gobject qoi-headers-git" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
libpython libavcodec.so libgudev libpoppler.so libhogweed.so libgjs.so girepository libgpgme.so libhwy.so libassuan.so $lib_audio_keywords $lib_browser_launcher"
