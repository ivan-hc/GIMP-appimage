...
echo y | ./.local/share/junest/bin/junest -- yay --answerclean All --answerdiff All --noconfirm -S aur/$(echo "$APP") --mflags "DISABLE_MESON=1"
...
mv ./*AppImage ./"$(cat ./$APP.AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g')"_GIT_"$VERSION"-archimage3.4-x86_64.AppImage
